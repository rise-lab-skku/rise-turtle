...
void setup()
{
  ...
  // nh.advertise(hw_error_L_pub);
  // nh.advertise(hw_error_R_pub);
  // nh.advertise(goalpwm_L_pub);
  // nh.advertise(goalpwm_R_pub);
  // nh.advertise(goalvel_L_pub);
  // nh.advertise(goalvel_R_pub);
  nh.advertise(ppwm_L_pub);
  nh.advertise(ppwm_R_pub);
  nh.advertise(pvel_L_pub);
  nh.advertise(pvel_R_pub);
...
/*******************************************************************************
* Callback function for wheel_control msg
*******************************************************************************/
void wheelControlCallback(const geometry_msgs::Vector3& wheel_control_msg)
{
  int mode = (int)wheel_control_msg.x;
  bool success = true;
  // Check Mode and Change Mode
  if(mode != motor_driver.getMode())
  {
    if(mode == MODE_VELOCITY)
    {
      success = motor_driver.change_mode((uint8_t)mode);
      if(success) { nh.loginfo("Mode change >> MODE_VELOCITY."); }
      else { nh.loginfo("Mode change >> MODE_VELOCITY [Fail]."); }
    }
    else if(mode == MODE_PWM)
    {
      success = motor_driver.change_mode((uint8_t)mode);
      if(success) { nh.loginfo("Mode change >> MODE_PWM."); }
      else { nh.loginfo("Mode change >> MODE_PWM [Fail]."); }
    }
    else
    {
      nh.loginfo("Mode change >> Invalid Mode Input.");
      success = false;
    }
  }
  ...
  if(success)
  {
    if(mode == MODE_VELOCITY)
    {
      // wheel_control_msg.y: LEFT
      // wheel_control_msg.z: RIGHT
      goal_velocity_from_cmd[LINEAR]  = ((float)wheel_control_msg.z + (float)wheel_control_msg.y) / 2.0;
      goal_velocity_from_cmd[ANGULAR] = ((float)wheel_control_msg.z - (float)wheel_control_msg.y) / WHEEL_SEPARATION;
    ...
    {
      last_pwm[LEFT] = (float)wheel_control_msg.y;
      last_pwm[RIGHT] = (float)wheel_control_msg.z;
      motor_driver.writePWM(last_pwm);
    }
  }
}

void leftPWMCallback(const std_msgs::Float32& left_pwm_msg)
{
  bool success = true;
  if(MODE_PWM != motor_driver.getMode())
  {
    success = motor_driver.change_mode((uint8_t)MODE_PWM);
    if(success) { nh.loginfo("Mode change >> MODE_PWM."); }
    else { nh.loginfo("Mode change >> MODE_PWM [Fail]."); }
  }
  if (success)
  {
    last_pwm[LEFT] = (float)left_pwm_msg.data;
    motor_driver.writePWM(last_pwm);
  }
}
void rightPWMCallback(const std_msgs::Float32& right_pwm_msg)
{
  bool success = true;
  if(MODE_PWM != motor_driver.getMode())
  {
  ...
  if (success)
  {
    last_pwm[RIGHT] = (float)right_pwm_msg.data;
    motor_driver.writePWM(last_pwm);
  }
}
...
void publishMode(void)
{
  mode_msg.data = (uint8_t)motor_driver.getMode();
  mode_pub.publish(&mode_msg);
}

// void publishTorque(void)
// {
//   torque_msg.data = (bool)motor_driver.getTorque();
//   torque_pub.publish(&torque_msg);
// }

// void publishHwError(void)
// {
//   bool result = motor_driver.readHwError(hw_error_L_msg.data, hw_error_R_msg.data);
//   hw_error_L_pub.publish(&hw_error_L_msg);
//   hw_error_R_pub.publish(&hw_error_R_msg);
// }


void publisPresent_PWM_Vel(void)
{
  bool result = false;
  result = motor_driver.readPresentPWM(present_pwm_L_msg.data, present_pwm_R_msg.data);
  if (result == true)
  {
    ppwm_L_pub.publish(&present_pwm_L_msg);
    ppwm_R_pub.publish(&present_pwm_R_msg);
  }
  result = motor_driver.readPresentVel(present_vel_L_msg.data, present_vel_R_msg.data);
  if (result == true)
  {
    pvel_L_pub.publish(&present_vel_L_msg);
    pvel_R_pub.publish(&present_vel_R_msg);
  }
}
